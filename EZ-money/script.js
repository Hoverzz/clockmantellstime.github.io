...
function num2txt(num) {
    ...
    if (num > 999 && num < 10000000000000000000) {
        if ((n.length-1) % 3 === 0) {
...
function reload() {
    ...
    document.getElementById("mk4").innerHTML = "Mk4 factory: an advanced factory that can produce $"+num2txt(stats.mk4Earn)+" every second, costs " + num2txt(prices.mk4) + "!";
    document.getElementById("mk5").innerHTML = "Mk5 factory: A super advanced factory that can print $"+num2txt(stats.mk5Earn) +" every second, but it isn't cheap!";
}     
...
function setDifficulty(diffiiculty) {
    if (confirm("Are you sure, you can't change the difficulty later?")) {
        for (var p in prices) {
            prices[p]  *=  difficulties[diffiiculty];
        }
        other.diffiiculty = diffiiculty;
...
function saveGame() {
    if (other.diffiiculty === "easy"){
        localStorage.setItem("easy",JSON.stringify(stats));
    }
    if (other.diffiiculty === "medium"){
        localStorage.setItem("medium",JSON.stringify(stats));
    }
...
function loadGame() {
    if (other.diffiiculty === "easy"){
        var data = JSON.parse(localStorage.getItem("easy"));
    }
    if (other.diffiiculty === "medium"){
        var data = JSON.parse(localStorage.getItem("medium"));
    }    
    if (other.diffiiculty === "hard"){
        var data = JSON.parse(localStorage.getItem("hard"));
    }  
    if (other.diffiiculty === "insane"){
        var data = JSON.parse(localStorage.getItem("insane"));  
    }    
    try{
        for (var x in data) {
            stats[x] = data[x];
